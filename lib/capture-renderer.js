...
getScreenSources({}, (imgSrc) => {
    ...
    $btnClose.addEventListener('click', () => {//關閉
        ipcRenderer.send('capture-screen', {
            type: 'close',
        })
        window.close()
    ...
    $btnReset.addEventListener('click', () => {//放棄重來
        capture.reset()
    })
    ...
    $btnOk.addEventListener('click', selectCapture)//複製到剪貼簿

    $btnSave.addEventListener('click', () => {//存檔
        let url = capture.getImageUrl()
